...
module Paperclip
  ...
  class Attachment
    ...
    def initialize name, instance, options = {}
      ...
      @permitted_file_types  = options[:permitted_file_types]
    ...
    def assign uploaded_file
      ...
      if uploaded_file.is_a?(String)
        if !uploaded_file.blank?
          uploaded_file = CGI.unescape(uploaded_file) #removes junk characters from sent uploaded address
          file_obj = uploaded_file.split("?")
          file_name = file_obj.first.split("/").last
          instance_write(:file_name, file_name)
          file_obj.last.split("&").each do |details|
            instance_write(:content_type,details.split("=").last) if details.split("=").include?("content_type")
            instance_write(:file_size,details.split("=").last) if details.split("=").include?("file_size")
          end
          instance_write(:updated_at,      Time.now)
          @queued_for_write[:s3_file_key] = file_obj.first.split("com/").last
        end
      else
        if uploaded_file.is_a?(Paperclip::Attachment)
#          uploaded_file = uploaded_file.to_file(:original)
          if @storage == :s3
            @queued_for_write[:s3_file_copy] = uploaded_file
          else
            uploaded_file = uploaded_file.to_file(:original)
          end
...
#        @queued_for_write[:original]   = to_tempfile(uploaded_file)
        @queued_for_write[:original]   = to_tempfile(uploaded_file) unless uploaded_file.is_a?(Paperclip::Attachment) and @storage == :s3
        instance_write(:file_name,       uploaded_file.original_filename.strip)
        instance_write(:content_type,    uploaded_file.content_type.to_s.strip)
        instance_write(:file_size,       uploaded_file.size.to_i)
#        instance_write(:fingerprint,     generate_fingerprint(uploaded_file))
        instance_write(:fingerprint,     generate_fingerprint(uploaded_file)) unless uploaded_file.is_a?(Paperclip::Attachment) and @storage == :s3
        instance_write(:updated_at,      Time.now)
...
#        instance_write(:fingerprint, generate_fingerprint(@queued_for_write[:original]))
        instance_write(:file_size,   @queued_for_write[:original].size.to_i) unless @storage == :s3
        instance_write(:fingerprint, generate_fingerprint(@queued_for_write[:original])) unless @storage == :s3
...
#      original_filename.nil? ? interpolate(@default_path, style_name) : interpolate(@path, style_name)
      CGI.unescape((original_filename.nil? ? interpolate(@default_path, style_name) : interpolate(@path, style_name)).gsub('+','%2B'))
    end
...
#      !original_filename.blank?
      ## validate if a file exists physically as per defined storage 
      exists?
    end
